# 👋 Hi, I'm Adarsh Rai

### 🐍 Python Developer | 🛰️ Network Engineer
**Bridging the gap between Code and Infrastructure.**

I specialize in **Python-driven network automation**, focusing on optimizing **Routing protocols** and building tools that make infrastructure more resilient and programmable.

---

### 🚀 At a Glance
- 🔭 **Focus:** Python Development & Advanced Routing (OSPF, BGP, Static).
- 🌱 **Learning:** Integrating AI with Network Monitoring & SDN.
- 🛠️ **Top Skill:** Python (Automation scripts, API integrations, Netmiko/NAPALM).
- 💬 **Ask me about:** Why I prefer automating a route change over doing it manually.

---

### 🧰 Tech Stack & Tools

| Category | Tools |
| :--- | :--- |
| **Programming** | ![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white) ![Bash](https://img.shields.io/badge/Shell_Script-4EAA25?style=flat-square&logo=gnu-bash&logoColor=white) |
| **Networking** | ![Cisco](https://img.shields.io/badge/Cisco-049FD9?style=flat-square&logo=cisco&logoColor=white) ![Juniper](https://img.shields.io/badge/Juniper_Networks-641D5E?style=flat-square&logo=junipernetworks&logoColor=white) **Routing & Switching** |
| **Automation** | ![Ansible](https://img.shields.io/badge/Ansible-EE0000?style=flat-square&logo=ansible&logoColor=white) ![Docker](https://img.shields.io/badge/Docker-2496ED?style=flat-square&logo=docker&logoColor=white) |

---

### 📊 My GitHub Stats
![Adarsh's Stats](https://github-readme-stats.vercel.app/api?username=YOUR_GITHUB_USERNAME&show_icons=true&theme=tokyonight)

---

### 📧 Connect with Me
- **LinkedIn:** [Adarsh Rai on LinkedIn](https://www.linkedin.com/in/adarsh-rai-1616302ba)
- **Email:** [adarshrai7607@gmail.com](mailto:adarshrai7607@gmail.com)
- **Portfolio:** *Coming Soon*

---
